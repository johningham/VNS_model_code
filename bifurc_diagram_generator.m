% Our last couple of efforts
% (composite_bifurcs/loop_NTS2TC_composite_bifurcs and
% flexi_comp_bifurcs/flexi_com_wrapper) were guilty of repeating lots of
% work. They actually only need to do n + m bifurc subplots, and not 2*m*n
% as they had been!

% To get the same effect, we should take our specs as in the previous
% wrappers, and generate (and store) subplots as MATLAB figures. 
% Bifurcation_VNS_takes_params should do still work here. 

% Can then do nested for loops and start loading appropriate figures to
% complile composite diagrams. Can put the vertical lines on at this stage.
% Can call plot_time_series at this point to get those too. 

% Can then appropriately name, label and save each composite diagram.

% Code below culled from flexi_comp_wrapper/flexi_comp_bifurcs:
...
wt2vary = [4, 11]; % e.g.[4,21] for NTS>TC
blurb = 'Amy2RE';
...
ex_min = -5; % default -5
ex_max = 5; % default 5
ex_step = 0.5; % default 0.5

inh_min = -8; % default -8
inh_max = 2; % default 2
inh_step = 0.5; % default 0.5


% % TEST PARAMETERS ONLY!! (override what is above)
% ex_step = 10;
% inh_step = 10; 
% weights = 0.09;
% blurb = 'test';
